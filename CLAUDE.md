...
| `routes`                | A planned outing on a specific day.                        | `route_date date not null`. `end_mode` (`round_trip` / `last_stop` / `address`, default `round_trip`) + optional `end_address`/`end_lat`/`end_lng` for the user's chosen end location. RLS: owner-only.                                                                                                                                                                                                              |
...
- Sale-photo storage cleanup happens client-side when the owner deletes the sale (best-effort). For _edits_, photo removals are staged via `PhotoUploader.commitPendingDeletes()` and only fire on submit success — so a user who removes a photo and then closes the page without saving doesn't end up with a broken-image reference.
...
21. `0021_sale_reports.sql` — persists every report submitted via `/api/sale-reports` in a new `sale_reports` table with a `unique (reporter_id, sale_id)` constraint (dedupes repeat reports of the same sale by the same user) plus indexes for reporter rate-limit lookups and per-sale admin lookups. RLS is on with no user-facing policies — the service-role endpoint is the only writer.
22. `0022_profiles_display_name_length.sql` — caps `profiles.display_name` at 80 chars via a CHECK constraint, truncates any existing oversize rows, and updates `handle_new_user()` to clamp the seeded value so an unusually long `raw_user_meta_data.display_name` can't trip the constraint at signup time.
23. `0023_route_end_options.sql` — adds `end_mode` (text, default `'round_trip'`, check constraint over `round_trip` / `last_stop` / `address`), `end_address`, `end_lat`, `end_lng` to `routes`. A check constraint forces the lat/lng to be present when `end_mode = 'address'`. Existing rows keep the legacy round-trip behavior.
...
- **Mapbox container resize.** `BrowseMap.vue` runs a `ResizeObserver` on its container and calls `map.resize()` on every change. Mapbox's built-in `trackResize` only listens to _window_ resizes, so without this the canvas would freeze at its initial size whenever internal layout shifts (mobile stacked detail+map, tab swaps) grew the container — pins past the original size would float over empty cream.
...
- **Modals support `data-autofocus`.** `useFocusTrap` looks for `[data-autofocus]` first when a modal opens, before falling back to the first focusable element. Lets a modal jump focus to its first _meaningful_ control (e.g. the reason `<select>` in `ReportSaleModal`) instead of the close-X.
...
- **Three route-end modes.** `/itineraries/[id]` exposes a 3-pill picker — **Round trip** (drive back to start), **Last sale** (end at the last dragged stop), or **An address** (end at a user-supplied geocoded address, e.g. a friend's house in a different town). State is persisted on the `routes` row (`end_mode` / `end_address` / `end_lat` / `end_lng`) so a refresh or share-link visitor sees the same choice. `useRouteOptimizer`'s `optimizeRoute`/`buildRouteFromOrder` now take a `RouteEnd { mode, coord? }` parameter; the result exposes both `returnLeg` (round-trip only) and `endLeg` (address-only) so the timeline can render an appropriate trailing row. The Mapbox Optimization 12-coord cap drops the stop limit from 11→10 when end mode is `address` (the end address burns one coord); the optimize-cap warning surfaces this dynamically. `/share/[id]` reads the saved `end_mode` and adjusts its mapsLinks accordingly — round-trip on share approximates as "return to first stop" since the viewer has no personal start point.
- **Maps deep-link cache-buster.** `/itineraries/[id]` and `/share/[id]`'s Google/Apple Maps buttons go through `openInMaps(url)` which appends `&_t=<Date.now()>` and uses `window.open`, while the `<a href>` keeps the static URL for right-click "open in new tab" accessibility. Without this, mobile OSes that delegate the link to the Maps app via intent tend to refresh the _previously-opened_ navigation activity rather than re-parse the new URL params — so a user who tweaks the route and re-taps the button sees the stale route. Unknown query params are ignored by Google/Apple Maps; the cache-buster's only job is to make the URI distinct enough that the OS treats it as a fresh intent.
...
- **Optimize-Order endpoint stays pinned.** When end mode is `'last_stop'`, the user's last dragged stop is the fixed endpoint (`destination=last`); Mapbox optimizes the middle. We deliberately did NOT implement a "let Mapbox pick which stop ends" mode — v1 doesn't support `destination=any roundtrip=false`, so faking it would mean running N optimizations (one per candidate endpoint) and picking the shortest, which burns API calls and adds latency. The picker is predictable instead: drag the stop you want last, and that's what gets pinned.
