...
- **Maps deep-link cache-buster.** `/itineraries/[id]` and `/share/[id]`'s Google/Apple Maps buttons go through `openInMaps(url)` which appends `&_t=<Date.now()>` and uses `window.open`, while the `<a href>` keeps the static URL for right-click "open in new tab" accessibility. Without this, mobile OSes that delegate the link to the Maps app via intent tend to refresh the *previously-opened* navigation activity rather than re-parse the new URL params — so a user who tweaks the route and re-taps the button sees the stale route. Unknown query params are ignored by Google/Apple Maps; the cache-buster's only job is to make the URI distinct enough that the OS treats it as a fresh intent.
