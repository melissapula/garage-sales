...
The app is a PWA — installable on iOS and Android via "Add to Home Screen" from the browser menu. Manifest (`manifest.webmanifest`) and service worker (`sw.js`) are generated by `@vite-pwa/nuxt` at build time. Runtime caching is configured for Mapbox tiles, Google Fonts, and Supabase sale photos so saved sales render offline. App-store distribution would require a Capacitor wrap on top of the same Nuxt build — not yet shipped.
