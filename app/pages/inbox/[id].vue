...
const { data, refresh } = await useAsyncData(`thread-${id}`, () => fetchThreadWithMessages(id), {
    watch: [user],
});
...
<template>
    <section class="mx-auto max-w-3xl px-4 py-6">
        ...
        <template v-else>
            <header class="mt-3 flex items-start justify-between gap-3">
                <div>
                    ...
                    <p v-if="data.thread.sale" class="mt-1 text-sm text-sky-700">
                        About:
                        <NuxtLink :to="`/sale/${data.thread.sale.id}`" class="hover:underline">
                            {{ data.thread.sale.title }}
                        </NuxtLink>
                    </p>
                </div>
                <button class="text-xs text-gray-600 hover:underline" @click="hideConversation">
                    Remove from inbox
                </button>
            </header>
            ...
            >
                <p v-if="data.messages.length === 0" class="py-8 text-center text-sm text-gray-500">
